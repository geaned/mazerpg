...
char maze[21][21];
int exp_border[102];
int save_data[73];   // DEPENDS ON ITEMS_ALL
...
unsigned long hash_33()
{
    ...
    for (int i = 0; i < 73; i++) hash_33 = ((hash_33 << 5) + hash_33) + save_data[i];
    return hash_33;
}
    ...
    if (endgame)
    {
        for (i = 0; i < int(v[17].size()); i++)
        {
            if (i%2 == 0)
            {
                ...
                cout << v[17][40+size_t(i/2)];
            }
            else
            {
                setCursorPosition(39-i/2, 1);
                cout << v[17][39-size_t(i/2)];
            }
            Sleep(52);
        }
        Sleep(500);
...
void battle(int type, int enemy_level, float diffC) // BATTLE
{
    ...
    switch(type)    // ENEMY GEN
    {
        case 0:
            ...
            myEnemy.max_hp = diffC*(0.1*enemy_level*enemy_level+100.*enemy_level+500-6.*enemy_level+rand()%max(1, 12*enemy_level));
            myEnemy.damage = diffC*(0.025*enemy_level*enemy_level+12.*enemy_level+30-enemy_level+rand()%max(1, 2*enemy_level));
            myEnemy.ult_id = -1;
            name_n = rand()%23;
            hp_chance = rand()%100;
        ...
        case 1:
            ...
            myEnemy.max_hp = 5*diffC*(0.1*enemy_level*enemy_level+100.*enemy_level+500-6.*enemy_level+rand()%max(1, 12*enemy_level));
            myEnemy.damage = diffC*(0.025*enemy_level*enemy_level+12.*enemy_level+30-enemy_level+rand()%max(1, 2*enemy_level));
        ...
        case 2:
            ...
            myEnemy.max_hp = 20*diffC*(0.1*enemy_level*enemy_level+100.*enemy_level+500-6.*enemy_level+rand()%max(1, 12*enemy_level));
            myEnemy.damage = diffC*(0.025*enemy_level*enemy_level+12.*enemy_level+30-enemy_level+rand()%max(1, 2*enemy_level));
            myEnemy.ult_id = 7;
            name_n = 29;
            hp_chance = 0;
...
int main()
{
    ...
    else
    {
        ...
        if (myHero.perks != 0)
        {
            ...
            save_data[0] = myHero.perks;
            save_data[1] = music_volume;
            save_data[2] = sound_volume;
            save_data[3] = unmute;
            save_data[23] = diff;
            save_data[24] = cnt;
            save_data[25] = x_s;
            ...
            save_data[29] = x_t;
            save_data[30] = y_t;
            save_data[31] = myHero.hp;
            save_data[32] = myHero.mp;
            save_data[33] = myHero.charge;
            save_data[34] = myHero.weapon_damage;
            save_data[35] = myHero.weapon_ap;
            save_data[36] = myHero.weapon_type;
            save_data[37] = myHero.weapon_level;
            save_data[38] = myHero.weapon_id;
            save_data[39] = myHero.weapon_rarity;
            save_data[40] = myHero.wear_armor;
            save_data[41] = myHero.wear_ap;
            save_data[42] = myHero.wear_type;
            save_data[43] = myHero.wear_level;
            save_data[44] = myHero.wear_id;
            save_data[45] = myHero.wear_rarity;
            save_data[46] = myHero.amulet_id;
            save_data[47] = myHero.amulet_rarity;
            save_data[48] = myHero.ring_id;
            save_data[49] = myHero.ring_rarity;
            save_data[50] = myHero.level;
            save_data[51] = myHero.exp;
            save_data[52] = myHero.gold;
            save_data[53] = quality;
            for (i = 0; i < items_all; i++)
            {
                fin >> myHero.backpack[i];
                save_data[54+i] = myHero.backpack[i];
            }
            fin >> checksum;
            checksumN = hash_33();
